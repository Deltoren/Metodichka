...
1. Изучить запросы, отправляемые с клиентской части. [Файл](./requests.txt).
...
В этом репозитории найдите файл `static\js\main...`. В нём необходимо поменять `url:"http://127.0.0.1:8000"` на адрес вашего сайта `url:"http://yourwebsiteurl.ru"`.

Список страниц соответствует таковому в файле с запросами:
* yoursite/auth
* yoursite/register
* yoursite/profile
* yoursite/profile/edit
 
Для остальных страниц есть только свёрстанный шаблон, но JS для них отсутствет. Для получения дополнительных баллов можно разработать и их. 
